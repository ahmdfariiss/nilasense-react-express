...
class MonitoringService {
  ...
  async getWaterQualityLogs(pondId, params = {}) {
    try {
      ...
      return {
        success: false,
        data: [],
        message: error.response?.data?.message || 'Gagal mengambil data monitoring'
      };
    }
  }
  ...
  async getLatestWaterQuality(pondId) {
    try {
      ...
      const latestData = response.data[0] || null;
      
      return {
        success: true,
        data: latestData,
        message: latestData ? 'Data terbaru berhasil diambil' : 'Belum ada data monitoring'
      };
    } catch (error) {
      ...
      return {
        success: false,
        data: null,
        message: error.response?.data?.message || 'Gagal mengambil data terbaru'
      };
    }
  }
  ...
  async addWaterQualityLog(pondId, data) {
    try {
      ...
      return {
        ...
        message: response.data.message || 'Data monitoring berhasil ditambahkan'
      };
    } catch (error) {
      ...
      return {
        success: false,
        data: null,
        message: error.response?.data?.message || 'Gagal menambahkan data monitoring'
      };
    }
  }
  ...
  async analyzeWaterQuality(data) {
    try {
      ...
      return {
        success: false,
        data: null,
        message: error.response?.data?.message || 'Gagal menganalisis kualitas air'
      };
    }
  }
  ...
  getStatistics(logs) {
    ...
    const parameters = ['temperature', 'ph_level', 'dissolved_oxygen', 'turbidity'];
    const averages = {};
    const trends = {};
    ...
    parameters.forEach(param => {
      ...
      const recentAvg = values.slice(0, half).reduce((sum, val) => sum + val, 0) / half;
      const oldAvg = values.slice(half).reduce((sum, val) => sum + val, 0) / (values.length - half);
